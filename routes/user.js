...
router.post('/dashboard/finalcon', async (req, res) => {
  ...
  try {
    ...
    const response = await axios.get(`https://v6.exchangerate-api.com/v6/798c796bcafd3218e57c180f/latest/${selectedCurrency}`);
...
router.get('/dashboard/dollarRates', async (req, res) => {
  try {
    ...
    const response = await axios.get('https://v6.exchangerate-api.com/v6/798c796bcafd3218e57c180f/latest/USD');
    const data = await response.data;
...
router.post('/dashboard/finalconvert', async (req, res) => {
  try {
    ...
    for (const currency in allRates) {
      const rate = parseFloat(allRates[currency]);
      if (!isNaN(rate) && finalDollarRate.hasOwnProperty(currency)) {
        const finalRate = rate * finalDollarRate[currency]; // Calculate the final rate
        console.log('finalRate', finalRate)
        
        const initialResultValue = parseFloat(initialConvertedResults[currency]);
        if (!isNaN(initialResultValue)) {
          const convertedValue = finalRate * initialResultValue;
...
router.post('/dashboard/autoconvert', async (req, res) => {
  ...
  try {
    const response = await axios.get(`https://v6.exchangerate-api.com/v6/798c796bcafd3218e57c180f/latest/NGN`);
    const data = await response.data;
...
router.post('/dashboard/autofinalconvert', async (req, res) => {
  ...
  try {
    ...
    const ngnResponse = await axios.get(`https://v6.exchangerate-api.com/v6/798c796bcafd3218e57c180f/latest/NGN`);
